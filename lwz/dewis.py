...
def extract_date_from_tournament(tournament_id: str) -> datetime:
...
def get_player_rating_at(pkz: int, ref_date: datetime) -> int:
    ...
    for participation, date in zip(participations, dates):
        if date > ref_date:
            return int_or_default(participation['dwzalt'], default=0)

    return int_or_default(participations[-1]['dwzneu'], default=0)
